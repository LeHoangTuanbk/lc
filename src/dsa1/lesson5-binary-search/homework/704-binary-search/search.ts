...
export function findLowerUpperBound(nums: number[], target: number): number[] {
  let low = 0,
    high = nums.length - 1,
    mid;
...
export function findUpperBound(nums: number[], target: number): number {
  let low = 0,
    high = nums.length - 1,
    mid;
...
export function findPredecessor(nums: number[], target: number): number {
  let low = 0,
    high = nums.length - 1,
    mid;
...
console.log(findPredecessor([-1, 0, 3, 5, 9, 12], 5));
